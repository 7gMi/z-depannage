    ...
    <section ref={sectionRef} className="py-16 sm:py-20 bg-[var(--bg-dark)] overflow-hidden">
      <div className="max-w-5xl mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <p className="text-white/75 max-w-xl mx-auto">{t('process.subtitle')}</p>
        </div>
            ...
            {STEPS_KEYS.map(({ icon: Icon, step, titleKey, descKey }, i) => (
              ...
              >
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 transition-all duration-500 group-hover:scale-105 ${
                  ...
                  <Icon size={28} className={`transition-colors duration-500 ${i <= activeStep ? 'text-[var(--accent-bright)]' : 'text-white/75'}`} />
                </div>
                <span className={`text-xs font-bold mb-2 transition-colors duration-500 ${i <= activeStep ? 'text-[var(--accent)]' : 'text-white/75'}`}>
                  ÉTAPE {step}
                </span>
                <h3 className={`font-heading font-bold text-lg mb-2 transition-colors duration-500 ${i <= activeStep ? 'text-white' : 'text-white/75'}`}>{t(titleKey)}</h3>
                <p className={`text-sm leading-relaxed transition-colors duration-500 ${i <= activeStep ? 'text-white/75' : 'text-white/75'}`}>{t(descKey)}</p>
          ...
          {STEPS_KEYS.map(({ icon: Icon, step, titleKey, descKey }) => (
            <div key={step} className="reveal flex gap-4">
              ...
              <div className="pb-6">
                ...
                <p className="text-sm text-white/75 leading-relaxed">{t(descKey)}</p>
              </div>
            </div>
          ))}
        </div>
