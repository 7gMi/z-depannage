...
export function Features() {
  ...
  return (
    <section className="relative py-12 sm:py-16 bg-[var(--bg-primary)] overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 reveal-stagger">
          {FEATURES_KEYS.map(({ icon: Icon, titleKey, descKey }) => (
            <div
              key={titleKey}
              className="icon-hover-parent reveal flex flex-col items-center text-center p-6 rounded-2xl border border-[var(--border-default)] bg-[var(--bg-card)] shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div className="icon-hover w-14 h-14 rounded-2xl flex items-center justify-center mb-4 bg-[var(--accent)]/8">
